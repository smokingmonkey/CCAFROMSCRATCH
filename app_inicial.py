...
sys.path.append('D:\\Andy\\CCAFROMSCRATCH\\datos\\codigo_base')
...
app_prueba.mount("/resultados", StaticFiles(directory="datos\\archivos\\resultados\\graficas"), name="graficas")
app_prueba.mount("/StylesFiles", StaticFiles(directory="FrontEnd\\Styles"), name="styles")
app_prueba.mount("/JavascriptFiles", StaticFiles(directory="FrontEnd\\Javascript"), name="globeJs")

@app_prueba.get("/")
async def pagina_base():
    return RedirectResponse("http://127.0.0.1:8000/Principal", status_code=301)

@app_prueba.get("/Principal", response_class=HTMLResponse)
...
@app_prueba.get("/pagina_extraccion", response_class=HTMLResponse)
...
@app_prueba.get("/pagina_transformacion", response_class=HTMLResponse)
...
@app_prueba.post("/pagina_transformacion")
...
@app_prueba.get("/pagina_procesamiento", response_class=HTMLResponse)
...
@app_prueba.post("/pagina_procesamiento", response_class=HTMLResponse)
...
@app_prueba.get("/pagina_mapas", response_class=HTMLResponse)
...
@app_prueba.get("/mapa_raster_base", response_class=HTMLResponse)
...
@app_prueba.get("/mapa_acumulacion_simple", response_class=HTMLResponse)
...
@app_prueba.post("/mapa_acumulacion_simple", response_class=HTMLResponse)
...
@app_prueba.get("/resultados_cuenca", response_class=HTMLResponse)
...
@app_prueba.get("/mapa_acumulacion_detalle", response_class=HTMLResponse)
...
@app_prueba.post("/mapa_acumulacion_detalle", response_class=HTMLResponse)
